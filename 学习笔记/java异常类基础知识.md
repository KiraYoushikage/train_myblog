...
### [Java 异常类层次结构图](https://snailclimb.gitee.io/javaguide/#/docs/java/basis/Java基础知识?id=java-异常类层次结构图)
...
- **`Error`** ：`Error` 属于程序无法处理的错误 ，我们没办法通过 `catch` 来进行捕获 。例如，Java 虚拟机运行错误（`Virtual MachineError`）、虚拟机内存不够错误(`OutOfMemoryError`)、类定义错误（`NoClassDefFoundError`）等 。这些异常发生时，Java 虚拟机（JVM）一般会选择线程终止，严格来说是jvm进程终止。

  >
  >
  >~~也就是说，一旦发生error是无可避免的造成程序崩溃，但是Exception不一定，Exception如果没有捕获，当前线程可能会终止，但不会影响到整个jvm进程，一旦捕获并处理的该Exception，则可以避免当前线程奔溃。~~ 
...
>
>
>这说明Error类型的异常也是可以被try catch 捕获到的
>
...
### [Throwable 类常用方法](https://snailclimb.gitee.io/javaguide/#/docs/java/basis/Java基础知识?id=throwable-类常用方法)
